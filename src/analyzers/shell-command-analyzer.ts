...
const SHELL_COMMANDS: Record<string, ShellCommand> = {
  ...
  ls: {
    ...
    alternatives: { windows: "dir" },
  },
  grep: {
    name: "grep",
    platforms: ["linux", "macos", "posix"],
    alternatives: { windows: "findstr" },
  },
  cat: {
    name: "cat",
    platforms: ["linux", "macos", "posix"],
    alternatives: { windows: "type" },
  },
  rm: {
    name: "rm",
    platforms: ["linux", "macos", "posix"],
    alternatives: { windows: "del" },
  },
  mv: {
    name: "mv",
    platforms: ["linux", "macos", "posix"],
    alternatives: { windows: "move" },
  },
  cp: {
    name: "cp",
    platforms: ["linux", "macos", "posix"],
    alternatives: { windows: "copy" },
  },
  ...
  ps: {
    name: "ps",
    platforms: ["linux", "macos", "posix"],
    alternatives: { windows: "tasklist" },
  },
  kill: {
    name: "kill",
    platforms: ["linux", "macos", "posix"],
    alternatives: { windows: "taskkill" },
  },
  which: {
    name: "which",
    platforms: ["linux", "macos", "posix"],
    alternatives: { windows: "where" },
  },
  ...
  tasklist: {
    name: "tasklist",
    platforms: ["windows"],
    alternatives: { unix: "ps" },
  },
  taskkill: {
    name: "taskkill",
    platforms: ["windows"],
    alternatives: { unix: "kill" },
  },
};
  ...
  private analyzeCommandLine(line: string, lineNumber: number): void {
    ...
    for (const cmd of commands) {
      ...
      if (cmdInfo) {
        ...
        const isUnix = cmdInfo.platforms.some((p) =>
          ["linux", "macos", "posix"].includes(p),
        );
        ...
        for (const match of matches) {
          ...
          if (!commonVars.includes(varName)) {
            this.issues.push({
              ...
              suggestion:
                "Check if environment variable exists before using it, or provide defaults",
            });
          }
        }
      }
  ...
  private analyzeShebang(shebang: string, lineNumber: number): void {
    if (shebang.includes("/bin/bash") || shebang.includes("/usr/bin/bash")) {
      this.issues.push({
        ...
        suggestion:
          "Use #!/usr/bin/env node for Node.js scripts or provide .bat/.ps1 alternatives for Windows",
      });
    }
  }
}
