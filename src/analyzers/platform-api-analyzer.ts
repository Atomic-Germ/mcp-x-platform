...
export class PlatformAPIAnalyzer {
        ...
        ImportDeclaration: (path) => {
          ...
          if (this.isPlatformSpecificImport(source)) {
            this.issues.push({
              ...
              suggestion: 'Consider using a cross-platform alternative or feature detection',
            });
          }
        },
  ...
  private getPlatformFromImport(moduleName: string): string {
    if (moduleName.includes('win32') || moduleName.includes('windows')) return 'windows';
    if (moduleName.includes('mac') || moduleName.includes('darwin') || moduleName.includes('fsevents')) return 'macos';
    if (moduleName.includes('linux') || moduleName.includes('inotify')) return 'linux';
    return 'unknown';
  }
  ...
  private getSuggestion(apiName: string): string {
    ...
    return suggestions[apiName] || 'Use feature detection and provide fallbacks';
  }
}
